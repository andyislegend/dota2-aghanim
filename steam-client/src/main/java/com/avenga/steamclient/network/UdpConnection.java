...
import  com.avenga.steamclient.enums.EUdpPacketType;
import com.avenga.steamclient.enums.ProtocolType;
import com.avenga.steamclient.enums.UdpState;
import  com.avenga.steamclient.generated.ChallengeData;
import  com.avenga.steamclient.generated.ConnectData;
...
public class UdpConnection extends Connection {
     ...
     */
    private volatile AtomicReference<UdpState> state;
    ...
    public UdpConnection(String clientName) {
        ...
        state = new AtomicReference<>(UdpState.DISCONNECTED);
    }
    ...
    public void disconnect() {
        ...
        if (state.get() != UdpState.DISCONNECTED && state.getAndSet(UdpState.DISCONNECTING) == UdpState.DISCONNECTED) {
            state.set(UdpState.DISCONNECTED);
        }

        // only notify if we actually applied the disconnecting state
        if (state.get() == UdpState.DISCONNECTING) {
    ...
    public void send(byte[] data) {
        if (state.get() == UdpState.CONNECTED) {
    ...
    private void receiveChallenge(UdpPacket packet) {
        if (!state.compareAndSet(UdpState.CHALLENGE_REQ_SENT, UdpState.CONNECT_SENT)) {
            return;
        }
        try {
    ...
    private void receiveAccept(UdpPacket packet) {
        if (!state.compareAndSet(UdpState.CONNECT_SENT, UdpState.CONNECTED)) {
            return;
        }
    ...
    private void receiveData(UdpPacket packet) {
        ...
        if (state.get() != UdpState.CONNECTED && state.get() != UdpState.DISCONNECTING) {
            return;
        }
    ...
    private class NetLoop implements Runnable {
        ...
        public void run() {
            ...
            if (currentEndPoint != null) {
                ...
                if (!state.compareAndSet(UdpState.DISCONNECTED, UdpState.CHALLENGE_REQ_SENT)) {
                    state.set(UdpState.DISCONNECTING);
            ...
            while (state.get() != UdpState.DISCONNECTED) {
                try {
                    try {
                        sock.receive(packet);
                        received = true;
                ...
                if (state.get() != UdpState.DISCONNECTED) {
                    sendPendingMessages();
                }
